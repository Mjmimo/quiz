...
function generateTopicQuestions(subject, topic, index, gradeLabel) {
  ...
  const base = generators[subject][topic](index, level, gradeLabel);
  const stamped = stampQuestionsByGrade(base, index, gradeLabel);
  return ensureFourQuestions(stamped, index, gradeLabel, subject, topic);
}
...
function stampQuestionsByGrade(questions, gradeIndex, gradeLabel) {
  return questions.map((item, idx) => ({
    ...item,
    question: `[${gradeLabel} · Set ${gradeIndex + 1}.${idx + 1}] ${item.question}`
  }));
}

function ensureFourQuestions(questions, gradeIndex, gradeLabel, subject, topic) {
